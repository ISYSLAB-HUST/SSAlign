...
def process_all_files_torch(basenames,dim,topk, device,cos_threshold):
    ...
    for basename in basenames:
        csv_file_path = f"/data/foldseek_database/Scope40/faiss/SVD{dim}/{basename}.result"
...
def plot_loss_curve(losses,dim,topk,cos_threshold):
    ...
    # 保存图片
    plt.savefig(f"/data/wanglei_workspace/ssalign/SaprotProject/Saprot/SaProt/Scope40/image/Linear/{dim}_{topk}_{cos_threshold}_losses.png")
    plt.close()
...
def plot_model_results(X, y, model, slope, intercept,dim,topk,cos_threshold):
    ...
    # 保存图片
    plt.savefig(
        f"/data/wanglei_workspace/ssalign/SaprotProject/Saprot/SaProt/Scope40/image/Linear/{dim}_{topk}_{cos_threshold}_linear.png")
    plt.close()
...
def main(dim,topk,cos_threshold):
    basenames = []
    file_dir = "/data/foldseek_database/Scope40/pdb"
