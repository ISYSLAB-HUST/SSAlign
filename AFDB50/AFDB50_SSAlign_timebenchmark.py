...
DEFAULT_QUERY_DIR = "../pdbData/pdb/AFDB50"
DEFAULT_QUERY_EXT = ".cif"
...
def resolve_query_paths(query_entries, query_dir, query_ext):
    """
    支持两种输入：
      1) 只给名字：AF-XXXX-F1-model_v4 -> query_dir/name + query_ext
      2) 直接给路径：/path/to/xxx.cif -> 原样使用
    """
    out = []
    for x in query_entries:
        x = x.strip()
        if not x:
            continue
        if ("/" in x) or x.endswith(".cif") or x.endswith(".pdb") or x.endswith(".mmcif"):
            out.append(x)
        else:
            out.append(os.path.join(query_dir, x + query_ext))
    return out
...
def _process_one_query(args):
    ...
    high_df = pd.DataFrame(high_rows, columns=["File1", "Cosine_Similarity", "Score"])
...
def main():
    ...
    ap.add_argument("--query_file_list_file", required=True)
    ap.add_argument("--faiss_index", required=True)
    ap.add_argument("--dim", type=int, required=True)  # 必须等于 index.d
    ap.add_argument("--prefilter_target", type=int, required=True)  # FAISS topK
    ap.add_argument("--prefilter_threshold", type=float, required=True)  # 分数阈值（例如 0.3）
    ap.add_argument("--max_target", type=int, required=True)  # 最终输出行数（<= prefilter_target）
    ...
    ap.add_argument("--foldseek_exec", default=DEFAULT_FOLDSEEK_PATH)
    ap.add_argument("--saprot_model", default=DEFAULT_SAPROT_MODEL_PATH)
    ap.add_argument("--mu_file", default=DEFAULT_MU_FILENAME)
    ap.add_argument("--w_file", default=DEFAULT_W_FILENAME)

    ap.add_argument("--lookup_file", default=DEFAULT_LOOKUP_FILE)
    ap.add_argument("--index_file", default=DEFAULT_INDEX_FILE)
    ap.add_argument("--seq_file", default=DEFAULT_SEQ_FILE)

    ap.add_argument("--query_dir", default=DEFAULT_QUERY_DIR)
    ap.add_argument("--query_ext", default=DEFAULT_QUERY_EXT)
    ...
    # A) query list -> paths
    query_entries = parse_list_from_file(args.query_file_list_file)
    query_paths = resolve_query_paths(query_entries, args.query_dir, args.query_ext)
    print(f"[START] queries_in_list={len(query_entries)} resolved_paths={len(query_paths)}")

    # B) query foldseek_seq + combined_seq
    t = time.time()
    valid_paths, foldseek_seq_list, combined_seq_list = generate_query_seqs(args.foldseek_exec, query_paths)
    ...
    print(f"[SAligner DONE] cost={time.time() - t:.2f}s")

    included = time.time() - time_start
    print(f"\n[INCLUDED TIME] (exclude: load_saprot/faiss/lookup/index) = {included:.2f}s")
...
[PRELOAD DONE] (not counted)
  load_saprot   = 12.32s
  load_faiss    = 519.38s
  load_lookup   = 37.19s
  load_index    = 64.61s
  load_muW      = 0.02s
  preload_total = 633.53s
[START] queries_in_list=1000 resolved_paths=1000
[QSEQ] n=1000 cost=22.86s
[EMB] emb_mat shape=(1000, 1280) cost=473.89s
[WHITEN] whitened shape=(1000, 1280) cost=0.22s
[SSAlign-Prefilter] topK=2000 cost=1147.61s   ----- 如果是GPU，这里可以大幅减少
[TASKS] n=1000 threshold=0.3 cost=0.56s
[STAGE2] saligner nproc=64 (only score<0.3) ...
[PROGRESS] 10/1000 last=AF-A6Q6B5-F1-model_v4 status=OK
......
[PROGRESS] 1000/1000 last=AF-Q8DQ56-F1-model_v4 status=OK
[SAligner DONE] cost=1070.84s
[INCLUDED TIME] (exclude: load_saprot/faiss/lookup/index) = 2715.98s
"""