...
# 配置索引参数
def create_faiss_index(dim, index_type, index_path, batch_size=100000, nlist=100, m=16, nbits=8):
    """
    ...
    # 获取所有 FASTA 文件
    file_paths = [f"../models/SSAlignDB/AFDB50/split_fasta_{i}_vector_whitend.fasta" for i in range(1, 41)]
...
if __name__ == "__main__":
    ...
    # 索引文件和名称文件路径
    index_path = f"../models/SSAlignDB/AFDB50/{args.index_type}/afdb50_{args.dim}_{args.index_type}_faiss.faiss"

    # 调用函数创建 Faiss 索引
    create_faiss_index(args.dim, args.index_type, index_path)
