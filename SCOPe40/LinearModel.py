...
def process_all_files_torch(basenames,dim,topk, device,cos_threshold):
    ...
    for basename in basenames:
        csv_file_path = f"../benchmarkData/SCOPe40/SSAlign/SVD{dim}/ssalign_prefilter/{basename}.result"
...
def plot_loss_curve(losses,dim,topk,cos_threshold):
    ...
    # 保存图片
    plt.savefig(f"../benchmarkData/SCOPe40/benchmark/{dim}_{topk}_{cos_threshold}_linear.png")
    plt.close()
...
def plot_model_results(X, y, model, slope, intercept,dim,topk,cos_threshold):
    ...
    # 保存图片
    plt.savefig(
        f"../benchmarkData/SCOPe40/benchmark/{dim}_{topk}_{cos_threshold}_linear.png")
    plt.close()
...
def main(dim,topk,cos_threshold):
    basenames = []
    file_dir = "/pdbData/pdb/SCOPe40/pdb"
