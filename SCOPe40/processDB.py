...
SwissProt_dir = "../pdbData/pdb/SCOPe40/"  # 原始文件
...
def SCOPe40_2_seq_and_orgin_embedding(full_dir, output_file):
    # 列出所有的文件
...
def whitening(input_file, output_file):
    ...
    mu_filename = "../models/SSAlignDB/SCOPe40/SCOPe40_whitening_mu.npy"
    W_filename = "../models/SSAlignDB/SCOPe40/SCOPe40_whitening_W.npy"
...
def build_faiss(whitening_file_path,dim,index_path):
    ...
    # 保存 npz（后续你原逻辑就能继续用）
    npz_path = f"../models/SSAlignDB/SCOPe40/SCOPe40_id_embedding.npz"
    np.savez(npz_path,
    ...
    print(f"索引已保存到 {index_path}，包含 {index.ntotal} 个向量")
...
if __name__ == "__main__":
    ...
    orgin_embedding_file = '../models/SSAlignDB/SCOPe40/SCOPe40_vector_results'
    ...
    全部保存在一个文件中，格式：
        文件名,combined_seq,vector
    """
    SCOPe40_2_seq_and_orgin_embedding(SwissProt_dir, orgin_embedding_file)

    """
        白化，并且转化所有的原始嵌入
    """
    whitening_file_path = '../models/SSAlignDB/SCOPe40/SCOPe40_vector_results_whitening'
    ...
    for dim in dims:
        build_faiss(whitening_file_path, f"../models/SSAlignDB/SCOPe40/SCOPe40_IndexFlatIP_{dim}_faiss.index")


    """
        组织为npz文件，方便后续使用
    """
    output_npz = "../models/SSAlignDB/SCOPe40/SCOPe40_id_Seq.npz"
