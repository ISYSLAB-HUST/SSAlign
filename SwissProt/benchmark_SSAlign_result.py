...
def SSAlign_prefilter_workflow(dim, gpu_id):
    ...
    embedding_npz = "../models/SSAlignDB/SwissProt/SwissProt_id_embedding.npz"

    # 0) 读 query 名单
    basenames = []
    with open("/100filenames.txt", "r", encoding="utf-8", errors="replace") as f:
        for line in f:
    ...
    # 3) 读索引并搬到 GPU
    faiss_index_file = f"../models/SSAlignDB/SwissProt/SwissProt_IndexFlatIP_{dim}_faiss.index"
    ...
    out_dir = f"../benchmarkData/SwissProt/SSAlign/SVD{dim}/ssalign_prefilter"
    ...
    for qid in basenames:
        ...
        tmalign_file = f"../benchmarkData/SwissProt/tmalign/{qid}.result"
...
def SSAlign_workflow(dim, faiss_topk, cos_threshold, final_number):
    seq_npz = "./models/SSAlignDB/SwissProt/SwissProt_id_Seq.npz"
    ...
    pre_dir = f"./benchmarkData/SwissProt/SSAlign/SVD{dim}/ssalign_prefilter"
    out_dir = f"./benchmarkData/SwissProt/SSAlign/SVD{dim}/ssalign"
    os.makedirs(out_dir, exist_ok=True)
