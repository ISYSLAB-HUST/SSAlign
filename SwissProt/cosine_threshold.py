...
"""
cos_threshold 阈值选多少合适？

这里不涉及到foldseek，因为 准确率是和tmalign对比

ssalign-prefilter 结果即可
"""
...
def cos_test(basenames, dim,topk,cos_threshold):
    ...
    for basename in basenames:
        csv_file_path = f"../benchmarkData/SwissProt/SSAlign/SVD{dim}/ssalign_prefilter/{basename}.result"
...
def main(dim,topk):
    ...
    thresholds = np.arange(0.1, 1.0, 0.001)  # 步长更小
    precisions = []
    recalls = []
    f1_scores = []
    ...
    plt.savefig(f"../benchmarkData/SwissProt/benchmark/{dim}_{topk}_cosine_threshold.png")
